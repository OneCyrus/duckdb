...
bool JSONMultiFileInfo::ParseOption(ClientContext &context, const string &key, const Value &value, MultiFileOptions &,
	...
	if (loption == "merge_duplicate_fields") {
		options.merge_duplicate_fields = BooleanValue::Get(value);
		return true;
	}
	return false;
}
...
void JSONMultiFileInfo::BindReader(ClientContext &context, vector<LogicalType> &return_types, vector<string> &names,
	...
	transform_options.case_insensitive_field_matching = options.merge_duplicate_fields;

	if (options.auto_detect && !options.merge_duplicate_fields) {
